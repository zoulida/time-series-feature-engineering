...
class GPLearnFeatureExtractor:
    ...
    def calculate_ic(self):
        ...
        print(f"计算整体IC值，数据形状: {ic_data.shape}")
        print(f"特征列: {self.feature_names}")
        
        for feature in self.feature_names:
            if feature in ic_data.columns:
                try:
                    # 计算相关系数
                    feature_data = ic_data[feature].fillna(0)
                    target_data = ic_data['target'].fillna(0)
                    
                    print(f"计算 {feature} 的IC值...")
                    print(f"  特征数据范围: [{feature_data.min():.4f}, {feature_data.max():.4f}]")
                    print(f"  目标数据范围: [{target_data.min():.4f}, {target_data.max():.4f}]")
                    
                    # 检查数据是否包含NaN
                    if feature_data.isna().any() or target_data.isna().any():
                        corr = 0
                        mi = 0
                    else:
                        try:
                            ...
                            print(f"  相关系数计算失败: {corr_error}")
                            corr = 0
                        
                        try:
                            mi = mutual_info_score(feature_data, target_data)
                            print(f"  互信息: {mi:.6f}")
                        except Exception as mi_error:
                            print(f"  互信息计算失败: {mi_error}")
                            mi = 0
                    ...
                    print(f"警告：特征 {feature} 的IC计算失败: {e}")
                    ic_results.append({
                        'time': 'overall',
                        'feature': feature,
                        'correlation': 0,
